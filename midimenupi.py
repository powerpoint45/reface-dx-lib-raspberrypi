...
class FileSelector(tk.Tk):
    def __init__(self):
        ...
        style = ttk.Style()
        style.configure("TCombobox", fieldbackground="black", background="black", foreground="white", width=10)
        self.midi_dropdown = ttk.Combobox(self.control_frame, textvariable=self.selected_midi_device, values=self.midi_devices, font="Verdana 10 bold", style="TCombobox")
        self.midi_dropdown.pack(side=tk.LEFT, padx=(4, 4), pady=4)
        ...
        self.path_label.pack(side=tk.LEFT, fill=tk.X, expand=True, padx=(3, 0), pady=0)
        ...
        button_style = {"bg": "#082233", "fg": "white", "activebackground": "#2a4c63"}
        self.parent_button = tk.Button(self.button_frame, text="Parent", command=self.go_to_parent_folder, height=2, width=5, **button_style)
        self.parent_button.pack(side=tk.LEFT, padx=(0, 0), pady=(2, 0))

        # Add a Bookmarks button to navigate to the bookmarks folder
        self.bookmarks_button = tk.Button(self.button_frame, text="Bookmarks", command=self.go_to_bookmarks, height=2, width=8, **button_style)
        self.bookmarks_button.pack(side=tk.LEFT, padx=(0, 0), pady=(2, 0))

        # Add "Add to Bookmarks" button
        self.add_to_bookmarks_button = tk.Button(self.button_frame, text="Add Bookmark", command=self.add_to_bookmarks, height=2, width=10, **button_style)
        self.add_to_bookmarks_button.pack(side=tk.LEFT, padx=(0, 0), pady=(2, 0))

        # Add buttons for "New Folder" and "Delete"
        self.new_folder_button = tk.Button(self.button_frame, text="New Folder", command=self.create_new_folder, height=2, width=8, **button_style)
        self.new_folder_button.pack(side=tk.LEFT, padx=(0, 0), pady=(2, 0))

        self.delete_button = tk.Button(self.button_frame, text="Delete", command=self.delete_selected_item, height=2, width=7, **button_style)
        self.delete_button.pack(side=tk.LEFT, padx=(0, 0), pady=(2, 0))
    ...
    def add_to_bookmarks(self):
        ...
        def create_new_folder_in_dialog(dialog):
            ...
            # Prompt for new folder name, using the main window as the parent
